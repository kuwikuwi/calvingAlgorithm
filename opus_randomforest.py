...
# 모델 정의 함수 - Random Forest와 Decision Tree 추가
def get_models():
    return {
...
def compare_balanced_unbalanced():
    ...
    # 변화율 계산 - 모든 모델에 대해
    df_comparison = []
    for model in ['LightGBM', 'XGBoost', 'GradientBoosting', 'RandomForest', 'DecisionTree']:
    ...
    # 엑셀 파일로 저장
    with pd.ExcelWriter('balanced_vs_unbalanced_comparison_enhanced.xlsx', engine='openpyxl') as writer:
        # Sheet 1: 전체 결과
        ...
        # Balanced Accuracy 최고 추가
        best_bal_acc = df_avg.loc[df_avg['Balanced_Acc_mean'].idxmax()]
        recommendations.append({
            'Metric': 'Highest Balanced Accuracy',
            'Best_Model': best_bal_acc['Model'],
            'Data_Type': best_bal_acc['Data_Type'],
            'Value': f"{best_bal_acc['Balanced_Acc_mean']:.3f}"
        })
        ...
        # Sheet 6: 모델별 순위표
        ranking_metrics = ['Sensitivity_mean', 'Specificity_mean', 'F1_Score_mean', 'AUC_mean', 'Balanced_Acc_mean']
        ranking_data = []
        
        for metric in ranking_metrics:
            df_sorted = df_avg.sort_values(metric, ascending=False)
        ...
        df_ranking.to_excel(writer, sheet_name='Model_Rankings', index=False)
    
    print("\n=== 결과 저장 완료 ===")
    print("파일명: balanced_vs_unbalanced_comparison_enhanced.xlsx")
    print("\n시트 구성:")
    ...
    print("6. Model_Rankings: 모델별 순위표")
    ...
    # 추가: 모델별 특성 중요도 분석 (Random Forest와 Decision Tree만 가능)
    print("\n=== 특성 중요도 분석 (RF & DT) ===")
    
    # 전체 데이터로 학습하여 특성 중요도 확인
    all_train_dfs = []
    for i in range(1, 6):
        df = pd.read_excel(f'./트레이닝데이터/training_fold_{i}.xlsx', usecols=keep_cols)
        all_train_dfs.append(df)
    
    all_train = pd.concat(all_train_dfs, ignore_index=True)
    X_all = enhanced_feature_engineering(all_train.drop(columns=['TF']))
    y_all = all_train['TF']
    
    # Random Forest와 Decision Tree의 특성 중요도
